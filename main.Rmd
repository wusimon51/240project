...
```{r, echo = F}
amazon_first = amazon %>% filter(amazon < partner) %>% select(starts_with('amazon'))
amazon_lm = lm(amazon_stars ~ amazon_reviews, data = amazon_first)
amazon_first = amazon_first %>% add_residuals(amazon_lm)
ggplot(amazon_first) + aes(amazon_reviews, resid) + geom_point() +
  scale_x_log10()+ geom_hline(yintercept = 0, color = 'red', linetype = 'dashed')
...
```{r, echo = F}
non_amazon_first = amazon %>% filter(partner < amazon) %>% select(starts_with('partner'))
non_amazon_lm = lm(partner_stars ~ partner_reviews, data = non_amazon_first)
non_amazon_first = non_amazon_first %>% add_residuals(non_amazon_lm)
ggplot(non_amazon_first) + aes(partner_reviews, resid) + geom_point()+
  scale_x_log10() + geom_hline(yintercept = 0, color = 'red', linetype = 'dashed')
