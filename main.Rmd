...
For each search query, there is an associated Amazon product, Partner product, Independent product ranking, and associated star rating and review count for each product type. 
...
|
| **Independent Product** : Third Party Sellers not under a partnership with Amazon
|       **Classification** : Non_Amazon
|
...
>We answer this by comparing general and specific search queries average position using boxplots between Amazon and Partner (Non_Amazon) products.
...
### Question 2
- What is the distribution of positions for Amazon products compared to Partner (Non_Amazon) products in search results?
...
amazon %>%
    ...
    ggtitle("Distribution of Positions for Non-Amazon Products") +
    xlab("Position on Amazon") +
    ylab("Frequency") +
...
#### Amazon Products
> We first filter products in which Amazon's position on the website is higher than the Partner (Non_Amazon) product. 
...
ggplot(amazon_first, aes(x = amazon_stars, y = amazon_reviews)) +
  ...
  ggtitle("Correllation of Amazon Products", 
          subtitle = "Where the product is ranked higher than Partner (Non_Amazon)")
```
...
#### Partner (Non_Amazon) products

Next, we do the same thing as before, but instead we look at products in which Partner (Non_Amazon) products came before Amazon.
...
ggplot(non_amazon_first, aes(x = partner_stars, y = partner_reviews)) +
  ...
  xlab("Star Rating") +
  ylab("Review Count") +
  ggtitle("Correllation of Partner (Non_Amazon) Products", 
...
This is further supported by Question 4 of our analysis, as our hypothesis testing comparing Amazon versus Partner (Non_Amazon) products showed that the mean difference between their positions is 0. In other words, Amazon and Partner (Non_Amazon) products are placed similarly. 
...
Based on Question 2 of our analysis, which suggested that Amazon does not market their products higher based on quality, and Question 1 which suggested that Amazon markets their general products higher than specific ones, we had hypothesized that Amazon products were only placed higher to get more foot traffic and would not hold up in quality when compared to Non-Amazon retailers. However, when we ran our hypothesis tests comparing the mean placement of Amazon vs Partner, we found strong similarity between the two. This disproves our hypothesis, instead suggesting that there are other factors at play that determine the placement of Amazon products on their website. In Question 3 we see that the correlation between number of stars and number of reviews is much stronger in products where Amazon was ranked higher than Partner (Non-Amazon) products. This could suggest that Amazon products are a more trustworthy brand, at the very least for the general products it offers. 
